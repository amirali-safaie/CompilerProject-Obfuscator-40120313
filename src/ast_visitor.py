...
class CodeReconstructionVisitor(MiniCVisitor):
    ...
    def visitIfStatement(self, ctx:MiniCParser.IfStatementContext):
        ...
        if not then_stmt.strip().startswith(self.get_indent() + "{"): # If not a block
             then_stmt = f"\n{self.get_indent()}{then_stmt.strip()}" if '\n' in then_stmt.strip() else f" {then_stmt.strip()}"
    ...
    def visitWhileStatement(self, ctx:MiniCParser.WhileStatementContext):
        ...
        if not body_stmt.strip().startswith(self.get_indent() + "{"):
             body_stmt = f"\n{self.get_indent()}{body_stmt.strip()}" if '\n' in body_stmt.strip() else f" {body_stmt.strip()}"
        return f"{self.get_indent()}while ({condition}){body_stmt}"

    def visitForStatement(self, ctx:MiniCParser.ForStatementContext):
        init_str = ""
        if ctx.variableDeclaration():
            # Remove trailing semicolon and indent from var decl for 'for' loop
            init_str = self.visit(ctx.variableDeclaration()).strip().rstrip(';')
        elif ctx.expressionStatement():
             # Remove trailing semicolon and indent
            init_str = self.visit(ctx.expressionStatement()).strip().rstrip(';')
        # else SEMI, init_str remains empty (correctly)

        condition_str = ""
        if ctx.expression(0): # First expression is condition
            condition_str = self.visit(ctx.expression(0))

        update_str = ""
        if ctx.expression(1): # Second expression is update
            update_str = self.visit(ctx.expression(1))
        ...
        return f"{self.get_indent()}for ({init_str}; {condition_str}; {update_str}){body_stmt}"
    ...
    def visitAssignmentExpression(self, ctx:MiniCParser.AssignmentExpressionContext):
        if ctx.ASSIGN():
            ...
            right = self.visit(ctx.assignmentExpression()) # The right side (recursive for chained assignments)
            return f"{left} = {right}"
        return self.visit(ctx.logicalOrExpression()) # Or just the logicalOrExpression if no assignment
    ...
    def visitEqualityExpression(self, ctx:MiniCParser.EqualityExpressionContext):
        ...
        if len(ctx.relationalExpression()) > 1:
            op = "==" if ctx.EQ() else "!=" # Assuming only one operator if multiple expressions
